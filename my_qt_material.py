...
template = './input/material.css.template'


# ----------------------------------------------------------------------
def build_stylesheet(theme='', invert_secondary=False, resources=[], extra={},path=None):
    """"""
    theme = get_theme(theme, invert_secondary,path=path)
    if theme is None:
        return None
    ...
    set_icons_theme(theme,path=local_path)
...
# ----------------------------------------------------------------------
def get_theme(theme_name, invert_secondary=False,path=None):
    if path:
        local_path = path
    else:
        local_path = os.path.abspath(__file__)
    if theme_name in ['default.xml', 'default_dark.xml', 'default', 'default_dark']:
        theme = os.path.join(os.path.dirname(
            local_path), 'themes', 'dark_teal.xml')
    elif theme_name in ['default_light.xml', 'default_light']:
        invert_secondary = True
        theme = os.path.join(os.path.dirname(
            local_path), 'themes', 'light_blue.xml')
    elif not os.path.exists(theme_name):
        theme = os.path.join(os.path.dirname(
            local_path), 'themes', theme_name)
    else:
        theme = theme_name
...
# ----------------------------------------------------------------------
def add_fonts(path=None):
    """"""
    if path:
        local_path = path
    else:
        local_path = os.path.abspath(__file__)
    fonts_path = os.path.join(os.path.dirname(local_path), 'fonts')
...
# ----------------------------------------------------------------------
def apply_stylesheet(app, theme='',path="None", style='Fusion', save_as=None, invert_secondary=False, resources=[], extra={}):
    """"""
    add_fonts(path)
    if style:
        try:
            ...
            pass
    stylesheet = build_stylesheet(theme, invert_secondary, resources, extra,path)
...
# ----------------------------------------------------------------------
def set_icons_theme(theme, resource=None, output=None,path=None):
    """"""
    if path:
        local_path = path
    else:
        local_path = os.path.abspath(__file__)
    try:
        theme =theme = get_theme(theme, path=path)
    except:
        pass

    if resource is None:
        resource = os.path.join(os.path.dirname(
            local_path), _resource)
